...
app.add_middleware(
    ...
    allow_origins=[
        "*"
    ],  # 모든 오리진 허용 (프로덕션에서는 특정 도메인으로 제한하는 것이 좋음)
...
class NodeRequest(BaseModel):
    ...
    external_activity_weight: float = 50.0  # 새로 추가된 외부 활동 가중치
    top_n: int = 1
...
ORIGINAL_DATA_PATH = os.path.join(
    "..", DATA_DIR, "transfers_1643003325000_1643097085000.csv"
)
...
def calculate_derived_features(df):
    ...
    try:
        ...
        external_sent_stats = (
            ...
            .agg(
                external_sent_tx_count=("amount", "count"),
        ...
        external_recv_stats = (
            ...
            .agg(
                external_recv_tx_count=("amount", "count"),
        ...
        sent = temp_df[["fromAddress", "hour"]].rename(
            columns={"fromAddress": "address"}
        )
        hourly = (
            sent.groupby(["address", "hour"]).size().rename("hour_count").reset_index()
        )
        ...
        date_stats = (
            ...
            .agg(
                first_date=lambda x: (
                    x.min().strftime("%Y-%m-%d") if not x.empty else None
                ),
                last_date=lambda x: (
                    x.max().strftime("%Y-%m-%d") if not x.empty else None
                ),
        ...
        total_stats["counterparty_count_sent"] = total_stats[
            "counterparty_count_sent"
        ].fillna(0)
        total_stats["counterparty_count_recv"] = total_stats[
            "counterparty_count_recv"
        ].fillna(0)

        # 거래금액 특성 추가
        total_stats["sent_tx_amount_mean"] = total_stats[
            "sent_tx_amount"
        ] / total_stats["sent_tx_count"].replace(0, np.nan)
        total_stats["recv_tx_amount_mean"] = total_stats[
            "recv_tx_amount"
        ] / total_stats["recv_tx_count"].replace(0, np.nan)
        total_stats["external_sent_tx_amount_mean"] = total_stats[
            "external_sent_tx_amount"
        ] / total_stats["external_sent_tx_count"].replace(0, np.nan)
        total_stats["external_recv_tx_amount_mean"] = total_stats[
            "external_recv_tx_amount"
        ] / total_stats["external_recv_tx_count"].replace(0, np.nan)

        # 외부 활동 관련 특성 추가
        total_stats["external_tx_ratio"] = (
            total_stats["external_sent_tx_count"]
            + total_stats["external_recv_tx_count"]
        ) / (total_stats["sent_tx_count"] + total_stats["recv_tx_count"]).replace(
            0, np.nan
        )

        # 체인 다양성 계산 (얼마나 많은 다른 체인과 거래했는지)
        chain_diversity_sent = (
            df.groupby("fromAddress")["toChain"]
            .nunique()
            .rename("chain_diversity_sent")
            .reset_index()
            .rename(columns={"fromAddress": "address"})
        )

        chain_diversity_recv = (
            df.groupby("toAddress")["fromChain"]
            .nunique()
            .rename("chain_diversity_recv")
            .reset_index()
            .rename(columns={"toAddress": "address"})
        )
        ...
        total_stats = (
            total_stats.reset_index()
            .merge(chain_diversity_sent, on="address", how="left")
            .merge(chain_diversity_recv, on="address", how="left")
            .set_index("address")
        )

        total_stats["chain_diversity_sent"] = total_stats[
            "chain_diversity_sent"
        ].fillna(0)
        total_stats["chain_diversity_recv"] = total_stats[
            "chain_diversity_recv"
        ].fillna(0)
        total_stats["total_chain_diversity"] = (
            total_stats["chain_diversity_sent"] + total_stats["chain_diversity_recv"]
        )
...
# 가중치 적용 및 상위 노드 선별 함수 - 외부 활동 가중치 추가
def apply_weights_and_get_top_nodes(
    derived_df,
    batch_quant_weight,
    tx_count_weight,
    tx_amount_weight,
    external_activity_weight,
    top_n=10,
):
    """가중치를 적용하여 상위 노드 선별 - 외부 활동 점수 추가"""
    try:
        # 데이터프레임 복사
        df = derived_df.copy()

        # 정규화할 특성 선택 - 외부 활동 관련 특성 추가
        features = [
            "hour_entropy",
            "active_days_count",
            "sent_tx_count",
            ...
            "counterparty_count_recv",
            "external_tx_ratio",
            "total_chain_diversity",
            "external_sent_tx_count",
            "external_recv_tx_count",
        ]
        ...
        for feature in features:
            if feature in df.columns:
                ...
                if max_val > min_val:  # 0으로 나누기 방지
                    df[f"{feature}_norm"] = (df[feature] - min_val) / (
                        max_val - min_val
                    )
                else:
        ...
        df["batch_quant_score"] = (
            ...
            + (df["sent_tx_count_norm"] / (df["counterparty_count_sent_norm"] + 0.001))
            * 0.2
        )

        # 거래 횟수 점수
        df["tx_count_score"] = (
            df["sent_tx_count_norm"] * 0.5 + df["recv_tx_count_norm"] * 0.5
        )

        # 거래량 점수
        df["tx_amount_score"] = (
            df["sent_tx_amount_norm"] * 0.5 + df["recv_tx_amount_norm"] * 0.5
        )

        # 외부 활동 점수 계산 (새로 추가)
        df["external_activity_score"] = (
            df["external_tx_ratio_norm"] * 0.4  # 외부 거래 비율
            + df["total_chain_diversity_norm"] * 0.3  # 체인 다양성
            + df["external_sent_tx_count_norm"] * 0.15  # 외부 송금 횟수
            + df["external_recv_tx_count_norm"] * 0.15  # 외부 수신 횟수
        )
        ...
        if total_weight > 0:  # 0으로 나누기 방지
            ...
            norm_external_activity_weight = external_activity_weight / total_weight
        else:
            norm_batch_quant_weight = norm_tx_count_weight = norm_tx_amount_weight = (
                norm_external_activity_weight
            ) = 0.25
        ...
        df["final_score"] = (
            ...
            + df["external_activity_score"] * norm_external_activity_weight
        )
...
def get_related_nodes(original_df, top_node_addresses):
    ...
    try:
        ...
        sent_related = original_df[original_df["fromAddress"].isin(top_node_addresses)][
            "toAddress"
        ].unique()

        # 상위 노드가 수신자인 거래에서 발신자 추출
        recv_related = original_df[original_df["toAddress"].isin(top_node_addresses)][
            "fromAddress"
        ].unique()
        ...
        related_nodes = (
            set(sent_related)
            .union(set(recv_related))
            .difference(set(top_node_addresses))
        )
...
# API 엔드포인트: 노드 데이터 가져오기 - 외부 활동 가중치 추가
@app.post("/get_nodes")
async def get_nodes(request: NodeRequest):
    try:
        ...
        # 3. 가중치 적용하여 상위 노드 추출 - 외부 활동 가중치 추가
        top_nodes_derived = apply_weights_and_get_top_nodes(
            ...
            request.external_activity_weight,  # 새로 추가된 매개변수
            request.top_n,
        )
        ...
        top_nodes_derived_ret = derived_df[
            derived_df["address"].isin(top_node_addresses)
        ]
        print("@@@ 7", top_nodes_derived_ret)

        # 8. 관련 노드의 파생 데이터 계산
        related_nodes_derived = derived_df[
            derived_df["address"].isin(related_node_addresses)
        ]
        ...
        top_nodes_derived_json = json.loads(
            top_nodes_derived_ret.to_json(orient="records")
        )
        related_nodes_derived_json = json.loads(
            related_nodes_derived.to_json(orient="records")
        )
